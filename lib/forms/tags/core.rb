...
module Forms
  module Tags
    module Core
      ...
      desc %{
        Render a @<textarea>..</textarea>@ tag to be used in a form
        <pre><code<r:form:textarea name="object[key]">...</r:form:textarea></code></pre>
      ...
      tag 'form:textarea' do |tag|
        Forms::Tags::Helpers.require!(tag,"form:textarea",'name')

        %{<textarea #{Forms::Tags::Helpers.attributes(tag)}>#{tag.expand}</textarea>}
      end
